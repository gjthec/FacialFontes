  ...
  implements OnInit, AfterContentChecked, OnDestroy
{
  ...
  private loadResorceWithLocalStorage() {
    ...
    const dataStoredInLocalStore =
      this.localStorageFormService.getDataFromLocalStorage(
        resourceId,
        className,
        this.currentAction
      );
    ...
    this.localStorageFormService.saveInLocalStorageOnEachChange(
      resourceId,
      className,
      this.resourceForm,
      this.currentAction
          ...
          error: (error) =>
            alert(
              this.translocoService.translate(
                'componentsBase.Alerts.readErrorMessage'
              )
            ),
        });
    }
    ...
    for (let field in resourceForm) {
      if (resourceForm[field] instanceof Object) {
        ...
        if (resourceForm[field] instanceof Array) {
          ...
          if (
            resourceForm[field].id == undefined ||
            resourceForm[field].id == null
          ) {
            continue;
          }
  ...
  protected actionsForSuccess(resource: T) {
    ...
    alert(
      this.translocoService.translate(
        'componentsBase.Alerts.defaultSuccessMessage'
      )
    );

    // redirect/reload component page
    this.router
      .navigateByUrl(baseComponentPath, { skipLocationChange: false })
      .then(
  ...
  protected actionsForError(error) {
    ...
    if (error.status === 400 || error.status === 500) {
      ...
      if (errorMessage && errorMessage.includes('Validation error')) {
        alert(
          this.translocoService.translate(
            'componentsBase.Alerts.errorMessageLimitation'
          )
        );
      } else {
        alert(
          errorMessage ||
            this.translocoService.translate(
              'componentsBase.Alerts.defaultErrorMessage'
            )
        );
      }
      return;
  ...
  alertToReturn() {
    ...
    alert(
      this.translocoService.translate('componentsBase.Alerts.rememberToSave')
    );
  }
