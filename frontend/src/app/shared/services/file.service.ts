...
export class FileService {
  ...
  // endpoint do backend que você mostrou
  private fileUrl = environment.backendUrl + '/api/file';
  ...
  downloadFile(fieldFile: IFieldFile, fileIndex: number) {
    ...
    const blob = new Blob([file.dataBlob], {
  ...
  downloadFilesFromId(fieldFileId: string) {
    this.http
      .get<IFieldFile>(`${this.url}/${fieldFileId}`)
      .subscribe((fieldFile) => {
  ...
  getDataUrlByFieldFileId(
    fieldFileId: number | string
  ): Observable<{ dataUrl: string }> {
    return this.http.get<{ dataUrl: string }>(
      `${this.fileUrl}/field-file/${fieldFileId}/data-url`
    );
  }
}
