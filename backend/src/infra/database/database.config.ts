...
export async function getTenantConnection(
  ...
  try {
    const tenantConnectionService: TenantConnectionService =
      TenantConnectionService.instance;
    ...
    const defaultTenantConnection: TenantConnection =
      await getSecurityTenantConnectionUseCase.execute();

    if (
      tenantConnectionService.checkUserPermissionTenant(
        userUID,
    ...
    const tenantConnection: TenantConnection | null =
      tenantConnectionService.findOneConnection(databaseCredentialId);

    if (tenantConnection == null) {
      //Realizar a conexão do tenant do usuário
      const databaseCredentialRepository: DatabaseCredentialRepository =
        new DatabaseCredentialRepository(defaultTenantConnection);
      ...
      if (databaseCredential == null) {
        throw new Error(
          "Erro to try find database credential to connect Tenant."
        );
      }
...
export async function connectTenant(
  ...
  if (
    databaseCredential.type == undefined ||
    databaseCredential.type == null ||
    databaseCredential.password == null
  ) {
    throw new Error(
  ...
  try {
    ...
    try {
      ...
      throw new Error(
        "Error to Save Routes on tenant Database. Detail: " + error
      );
    }
    ...
    try {
      ...
      throw new Error(
        "Error to Save Menu on tenant Database. Detail: " + error
      );
    }

    const tenantConnectionService: TenantConnectionService =
      TenantConnectionService.instance;
    tenantConnectionService.setOnTenantConnectionPool(
