...
export class FieldFileController {
  ...
  async upload(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      console.log("aqui esta", req.body);
      //O Service será criado com base no tipo de banco de dados e o model usado
      const fieldFileRepository: FieldFileRepository = new FieldFileRepository(
        req.body.tenantConnection as TenantConnection
      );
      ...
      const { fieldType, files, location } = req.body;

      // Criar uma nova variável com apenas os dados desejados
      const fieldFile = { fieldType, files, location };
  ...
  async findAllFilesById(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const fieldFileWithFiles = await fieldFileRepository.findAllFilesById(
        fieldFileId
      );
