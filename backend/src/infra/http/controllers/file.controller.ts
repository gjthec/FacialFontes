...
export class FileController {
  ...
  async findDataUrlByFieldFileId(
    req: Request,
    res: Response,
    next: NextFunction
  ) {
    try {
      ...
      if (!id) throw new NotFoundError("ID inválido.");

      const fileRepository: FileRepository = new FileRepository(
        req.body.tenantConnection as TenantConnection
      );

      const dataUrl = await fileRepository.findDataUrlByFieldFileId(id);
      if (!dataUrl)
        throw new NotFoundError("Arquivo não encontrado para esse FieldFile.");
